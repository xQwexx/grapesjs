 ...
 */
 ...
 import CommandWrapper, { CommandAbstract } from "./view/CommandAbstract";
 ...
 export default class CommandsModule extends Module<CommandsConfig> {
   commands: {
     [id: string]: CommandWrapper
   } = {};
   defaultCommands: CommandWrapper[] = [];
   active: { [id: string]: CommandWrapper } = {};
   ...
   constructor(em: EditorModel) {
     ...
     const dfc = this.defaultCommands;
 
     dfc.push(new CommandWrapper("tlb-delete", this.config, {
       run(ed) {
         return ed.runCommand("core:component-delete");
       }
     }));
 
     dfc.push(new CommandWrapper("tlb-clone", this.config, {
       run(ed) {
     ...
     dfc.push(new CommandWrapper("tlb-move", this.config, {
       run(ed, sender, opts = {}) {
         let dragger;
     ...
     // Core commands
     dfc.push(new CommandWrapper("core:undo", this.config, {run:e => e.UndoManager.undo()}));
     dfc.push(new CommandWrapper("core:redo", this.config, {run:e => e.UndoManager.redo()}));
     commandsDef.forEach(item => {
       const oldCmd = item[2];
       const cmd = require(`./view/${item[1]}`).default;
       const cmdName = `core:${item[0]}`;
       dfc.push(new CommandWrapper(cmdName, this.config, cmd));
       if (oldCmd) {
         dfc.push(new CommandWrapper(oldCmd, this.config, cmd));
   ...
   add(id: string, obj: any | Function) {
     ...
     this.commands[id] = new CommandWrapper(id, this.config, obj);
     return this;
   }
    ...
    * */
   loadDefaultCommands() {
     this.defaultCommands.forEach(
       command => this.add(command.id, command)
     )
   ...
   create(command: any) {
     ...
     const cmd = CommandWrapper.extend(command);
