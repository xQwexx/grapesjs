...
export default class SelectComponent extends CommandAbstract {
   ...
   * @private
   * */
  toggleSelectComponent(enable = false) {
    const { em } = this;
  ...
  updateLocalPos(data?: HTMLElement) {
  ...
  toggleToolsEl(on = false, view?: HTMLElement, opts: any = {}) {
   ...
   * @param {Object} pos
   */
  hideElementOffset(view: HTMLElement) {
  ...
  updateBadge(el, pos, opts: any = {}) {
    ...
    if (!opts.posOnly) {
      ...
      const clsBadge = `${this.ppfx}badge`;
   ...
   * @private
   */
  showHighlighter(view: HTMLElement) {
    //@ts-ignore
  ...
  initResize(elem: any) {
    ...
    var toggleBodyClass = (method, e, opts: any) => {
    ...
    if (editor && resizable) {
      ...
      options = {
        ...
        updateTarget(el: HTMLElement, rect, options: any = {}) {
          if (!modelToStyle) {
            return;
          }
  ...
  updateToolsLocal(data?: any) {
    ...
    const topOff = frameOff?.top ?? 0;
    const leftOff = frameOff?.left ?? 0;
  ...
  stop(wrapper: Editor, sender: any, opts: any = {}) {
    if (!hasWin()) return;
    const { em, editor } = this;
    this.onHovered(); // force to hide toolbar
    this.stopSelectComponent();
    !opts.preserveSelected && em?.setSelected(undefined);
    this.toggleToolsEl();
    editor.stopCommand('resize');
  }
};
