...
export default class ShowOffsets extends CommandAbstract{
  getOffsetMethod(state: string) {
    var method = state || '';
    return 'get' + method + 'OffsetViewerEl' as keyof CanvasModule;
  }
  ...
  run(editor: Editor, sender: any, opts: any={}) {
    ...
    var pos = { ...(opt.elPos || this.canvas.getElementPos(el)) };
    ...
    var offsetViewer = this.canvas[method](opts.view);
